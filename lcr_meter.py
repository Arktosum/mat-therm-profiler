...
class LCRMeterGUI:
    def __init__(self, root):
        ...
        ttk.Label(conn_frame, text="Select Device:").grid(row=0, column=0, sticky="w")
        
        self.addr_combo = ttk.Combobox(conn_frame, width=35)
        self.addr_combo.grid(row=1, column=0, pady=5, padx=5)
        
        self.btn_refresh = ttk.Button(conn_frame, text="Refresh", command=self.refresh_devices)
        self.btn_refresh.grid(row=1, column=1, padx=2)

        self.btn_connect = ttk.Button(conn_frame, text="Connect", command=self.connect_instrument)
        ...
        # ============================================
        ctrl_frame = ttk.LabelFrame(root, text="Measurement Control", padding=10)
        ctrl_frame.pack(fill="x", padx=10, pady=5)

        ttk.Label(ctrl_frame, text="Frequency (Hz):").grid(row=0, column=0, sticky="w")
        ...
        self.btn_measure = ttk.Button(ctrl_frame, text="Set Freq & Measure", command=self.measure_reading, state="disabled")
        self.btn_measure.grid(row=1, column=0, columnspan=2, pady=10)

        ttk.Label(ctrl_frame, text="Main Output:").grid(row=2, column=0, sticky="nw")
        self.result_text = tk.Text(ctrl_frame, height=4, width=40, state="disabled", bg="#f0f0f0")
        self.result_text.grid(row=3, column=0, columnspan=2, pady=5)

        # ============================================
        # SECTION 3: DEBUG / COMMAND SENDER (NEW)
        # ============================================
        debug_frame = ttk.LabelFrame(root, text="Debug / Command Sender", padding=10)
        debug_frame.pack(fill="both", expand=True, padx=10, pady=5)

        ttk.Label(debug_frame, text="SCPI Command:").grid(row=0, column=0, sticky="w")
        ...
        self.btn_send = ttk.Button(debug_frame, text="Send", command=self.send_manual_cmd, state="disabled")
        self.btn_send.grid(row=0, column=2, padx=5)

        ttk.Label(debug_frame, text="Debug Response:").grid(row=1, column=0, sticky="nw", pady=(10,0))
        self.debug_text = tk.Text(debug_frame, height=5, width=40, state="disabled", bg="#e6e6e6", font=("Consolas", 9))
        self.debug_text.grid(row=2, column=0, columnspan=3, pady=5, sticky="nsew")
    ...
    def refresh_devices(self):
        try:
            ...
            if devices: self.addr_combo.current(0)
            else: self.addr_combo.set("No devices found")
    ...
    def connect_instrument(self):
        try:
            ...
            self.lbl_status.config(text=f"Connected: {idn.strip()}", foreground="green")
    ...
    def measure_reading(self):
        ...
        try:
            ...
            self.inst.write(":MEASure:ITEM 255,255,255")
            self.inst.write(":MEASure:VALid 1")
            
            # 3. Trigger and Read
            # Note: This sequence forces external trigger mode, triggers, then measures.
            reading = self.inst.query(":TRIGger EXTernal;*TRG;:MEASure?")
