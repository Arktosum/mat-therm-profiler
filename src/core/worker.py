...
class ExperimentWorker(threading.Thread):
    """
    Runs the full experiment sequence in a background thread.

    Modes:
      MODE_TEMP_ONLY  — single continuous ramp from start to end temp, no steps,
                        no dwell, hold at end temp when reached.
      MODE_FULL_SWEEP — step ramp with dwell + LCR frequency sweep at each step.
    """
    ...
    def _log_row(self, sp: float, pv: float, wsp: float):
        """Write one CSV row for Temp Only mode."""
        ts_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with open(self.csv_file, 'a', newline='', encoding='utf-8') as f:
            csv.writer(f).writerow([ts_str, sp, pv, wsp])

    # -------------------------------------------------------------------------
    # MODE_TEMP_ONLY: single continuous ramp, stop + hold at end temp
    # -------------------------------------------------------------------------
    ...
    def _run_temp_only(self, oven: EurothermDriver, start_time: float):
        ...
        self._write_log(
            f"Ramping from {start_temp}°C to {end_temp}°C "
            f"at {rate}°C/min"
        )

        # Send the final target in one go — Eurotherm ramp rate handles the rest
        oven.set_sp(end_temp)

        # --- Ramp phase: poll until PV reaches end_temp ---
        while self.running:
            pv = oven.get_pv()
            wsp = oven.get_wsp()
            ...
            # Estimate time remaining based on ramp rate
            remaining_deg = max(end_temp - pv, 0)
            time_rem = (remaining_deg / rate) * 60 if rate > 0 else 0
            self.update_progress(
                f"Ramping... {pv:.1f}°C → {end_temp:.1f}°C", time_rem, pv, end_temp)

            if pv >= end_temp - tolerance:
                self._write_log(
                    f"Target {end_temp}°C reached (Actual: {pv:.1f}°C). Holding.")
                break

            time.sleep(1.0)

        # --- Hold phase: stay at end temp until stopped ---
        if self.running:
            while self.running:
                pv = oven.get_pv()
                wsp = oven.get_wsp()
                ...
                self._log_row(end_temp, pv, wsp)
                self.update_progress(
                    f"Holding @ {end_temp:.1f}°C", 0, pv, end_temp)

                time.sleep(1.0)

    # -------------------------------------------------------------------------
    # MODE_FULL_SWEEP helpers
    # -------------------------------------------------------------------------

    def _wait_for_stability(self, oven: EurothermDriver, sp: float,
                            is_heating: bool, total_temps: int,
                            t_idx: int, total_freqs: int,
                            start_time: float):
        """Block until PV is within tolerance of SP."""
        rate = self.config['ramp_rate']
        while self.running:
            pv = oven.get_pv()
            wsp = oven.get_wsp()
            elapsed = time.time() - start_time
            self.update_live_plot(elapsed, pv, sp, wsp)

            stable = (
                (is_heating and pv >= sp - self.config['tolerance']) or
                (not is_heating and pv <= sp + self.config['tolerance'])
            )
            if stable:
                self._write_log(
                    f"Target {sp}°C reached (Actual: {pv:.1f}°C). Starting dwell.")
                return

            rem_steps = total_temps - t_idx
    ...
    def _dwell(self, oven: EurothermDriver, sp: float,
               total_temps: int, t_idx: int,
               total_freqs: int, start_time: float):
        ...
        for i in range(dwell_sec):
            ...
            time_rem = (dwell_sec - i) + \
                       (total_temps - t_idx - 1) * \
                (dwell_sec + total_freqs * 0.3)
            self.update_progress(f"Dwelling @ {sp:.1f}°C...", time_rem, pv, sp)
            time.sleep(1)

    def _sweep_frequencies(self, lcr: LCRDriver, oven: EurothermDriver,
                           sp: float, freqs,
                           total_temps: int, t_idx: int,
                           dwell_sec: int, start_time: float):
    ...
    def run(self):
        ...
        try:
            self._write_log(
                f"=== EXPERIMENT STARTED [{self.mode.upper()}] ===")
            ...
            # ── FULL SWEEP ─────────────────────────────────────────────────
                ...
                freqs = np.logspace(
                    log_min, log_max,
                    num=int((log_max - log_min) *
                            self.config['steps_per_decade']) + 1
                )
                temps = np.arange(
                ...
                for t_idx, sp in enumerate(temps):
                    ...
                    self._wait_for_stability(
                        oven, sp, is_heating, total_temps,
                        t_idx, total_freqs, start_time)
                    self._dwell(
                        oven, sp, total_temps, t_idx, total_freqs, start_time)
                    if self.running:
                        self._sweep_frequencies(
                            lcr, oven, sp, freqs,
                            total_temps, t_idx, dwell_sec, start_time)
